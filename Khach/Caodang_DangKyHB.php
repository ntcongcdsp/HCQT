	...
	if(isset($_POST['submit']))
	{
		...
		if($_POST['txtMaBaoVe'] == $_SESSION['security_code'])
		{
			...
			if($conn->multi_query($sql) === TRUE)
			{
				...
				echo "<h3>Bạn đã đăng ký xét tuyển Trường cao đẳng sư phạm Thừa Thiên Huế thành công. </h3>Trường sẽ sớm liên lạc với bạn <a href='index.php?menu=CaoDang_HB'>Nhấp vào đây để vào trang Hướng dẫn đăng ký xét tuyển bằng học bạ cấp 3.</a>
			<meta http-equiv='refresh' content='1;url=index.php?menu=CaoDang_HB' />";
				exit;
			}
			else
			{
	...
	<div class="col-xs-12">
		<div class="panel panel-primary">
    		<div class="panel-heading" style="text-align: center; font-family: time new roman; font-size:17px;">
            	<b>ĐĂNG KÝ XÉT TUYỂN CAO ĐẲNG DỰA VÀO HỌC BẠ</b>
                <?php
					require_once("/PHP/ConnectDB.php");
					$conn = ConnectDB::connect();
				?>
				...
				<div class="row" style="position:relative;right:20px;">
                    ...
                    <div class="form-group" id="NganhHB">
    					...
    					<div class="col-sm-7">
							...
							<?php	
                                $conn = ConnectDB::connect();
                                $sql = "SELECT * FROM Nganh WHERE HeDaoTao = 1";
                                $resut = mysqli_query($conn, $sql);
                                if($resut->num_rows >0)
                                {
                                    while ($row = $resut->fetch_assoc())
                        	...
                        	<select class="form-control" name="slToHopMonHB" id="slToHopMonHB">
                                ...
                                <table>
                                	<tr>
                                    	...
                                    	<td><label for="txtMon3" style="font-size:14px" >ĐTB Môn 3: </label></td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td><input type="text" class="form-control" name="txtMon3" id="txtMon3" value="<?php echo $mon3; ?>"></td>
                                    </tr>
                                </table>
    					</div>
  					</div>
                    ...
                    <div class="form-group" align="center">
                        ...
                        <a href="Index.php?menu=CaoDang_HB"><button type="button" class="btn btn-default">Trở về</button></a>
                    </div>
                </form>
                </div>
			</div>
...
$(document).ready(function(){
	...
	$('#txtMon1').focusout(function(){
	 ...
	 $('#txtMon3').focusout(function(){
		 var value  = this.value;
		 if (value)
			{
			   if (!isFloat(value) || !$.isNumeric(value) || !inRange(value)) {
				  alert('1/Điểm  ĐTB môn 3 nhập vào phải là giá trị số và nằm trong khoảng từ 0 đến 10.\n2/Thay đổi dấu phẩy (,) hoặc dấu chấm (.) cho điểm lẻ nếu còn xuất hiện thông báo này');
				  $('#txtMon3').focus(); 
			   }
			}     
