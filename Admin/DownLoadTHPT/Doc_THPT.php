...
?>
	...
	<div class="col-md-12" align="right">
        <p><a href="../admin/Index.php?menu=DownLoadTHPT&amp;chucnang=DownLoad"><input type="button" class=" btn btn-primary" value="Tải dữ liệu"/></a> </p>
    </div>
	<table class="table table-condensed">
        ...
        <?php
            ...
            $sql = "SELECT A.MaTS, Ho, Ten, NgaySinh, C.MaNganh, TenNganh, MaToHop, NgayDK FROM ThiSinh AS A INNER JOIN dangkycd AS B ON A.MaTS = B.MaTS INNER JOIN nganh AS C ON B.MaNganh = C.MaNganh ORDER BY MaTS DESC";
            ...
            if($result->num_rows > 0)
            {
                while($row = $result->fetch_assoc())
                {
                    echo "<tr>";
                        ...
                        echo "<td> 
                            <a href='index.php?menu=DownLoadTHPT&chucnang=Sua&ID=".$row['MaTS']."'><input type='button' value='Sửa' class='btn btn-success'/></a>
                            <a href='index.php?menu=DownLoadTHPT&chucnang=Xoa&ID=".$row['MaTS']."'> <input type='button' value='Xóa' class='btn btn-danger'/> </a>
                            </td>";
                    echo "</tr>";
                }
            }
		...
		for($i=1;$i<=$page;$i++)
		{
		 if ($page_cr!=$i) echo "<a href='index.php?menu=DownLoadTHPT&start=".$row_per_page*($i-1)."'><button type='button' class='btn btn-success'>".$i."</button></a>";
