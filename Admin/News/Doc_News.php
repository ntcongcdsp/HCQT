...
?>
	...
	<div class="col-md-12" align="right">
        <p><a href="Index.php?menu=News&chucnang=Tao"><input type="button" class=" btn btn-primary" value="Tạo tin tức mới"/></a> </p>
    </div>
	<table class="table table-condensed">
            ...
            if($result->num_rows > 0)
            {
                while($row = $result->fetch_assoc())
                {
                    echo "<tr>";
                        ...
                        echo "<td> 
                            <a href='index.php?menu=News&chucnang=Sua&ID=".$row['ID']."'><input type='button' value='Sửa' class='btn btn-success'/></a>
                            <a href='index.php?menu=News&chucnang=Xoa&ID=".$row['ID']."'> <input type='button' value='Xóa' class='btn btn-danger'/> </a>
                            </td>";
                    echo "</tr>";
                }
            }
		...
		for($i=1;$i<=$page;$i++)
		{
		 if ($page_cr!=$i) echo "<a href='index.php?menu=News&start=".$row_per_page*($i-1)."'><button type='button' class='btn btn-success'>".$i."</button></a>";
